...
func (s *Service) ListProviders(ctx context.Context, payload ListProvidersPayload) ([]ProviderRecord, error) {
	slog.Debug("ListProviders called", "active_only", payload.ActiveOnly)
...
func (s *Service) CreateProvider(ctx context.Context, payload CreateProviderPayload) (ProviderRecord, error) {
	slog.Debug("CreateProvider called", "name", payload.Name, "type", payload.Type, "url", payload.URL)
	...
	if err != nil {
		slog.Error("CreateProvider SQL error", "err", err)
	...
	slog.Debug("CreateProvider success", "id", id)
...
func (s *Service) UpdateProvider(ctx context.Context, payload UpdateProviderPayload) (ProviderRecord, error) {
	slog.Debug("UpdateProvider called", "id", payload.ID, "name", payload.Name)
	if payload.ID <= 0 {
	...
	if err != nil {
		slog.Error("UpdateProvider SQL error", "id", payload.ID, "err", err)
		return ProviderRecord{}, fmt.Errorf("update provider: %w", err)
	}
	slog.Debug("UpdateProvider success", "id", payload.ID)
...
func (s *Service) DeleteProvider(ctx context.Context, payload DeleteProviderPayload) (map[string]any, error) {
	slog.Debug("DeleteProvider called", "id", payload.ID)
	if payload.ID <= 0 {
